	...
	<body>
							...
							<div class="user-profile layout-spacing">
								<div class="widget-content widget-content-area">
                                    ...
                                    <div class="row p-3">
										...
										<div class="col-md-12">
                                            <span><strong style="color:black">Total Marks: </strong> {{$quiz->total_marks}}</span>
                                        </div>
                                        <div class="col-md-12">
						...
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 layout-top-spacing">
							...
							<div class="col-lg-12 col-12 ">
									<div class="statbox widget box box-shadow mb-4">
										...
										<form action="{{url('/admin/uplaod_quiz_docs')}}" enctype="multipart/form-data" class="dropzone" id="dropzonewidget">
											</form>
										</div>
									</div>
								</div>
                                        ...
                                        <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th>Student Name</th>
                                                    <th class="dt-no-sorting">Upload Time</th>
                                            ...
                                            <tbody>
												@php
												 $count=1;
												@endphp
												@foreach($quiz_response as $response)
                                                <tr>
                                                    <td>{{$count}}</td>
                                                    <td>{{$response->student_detail->name}}</td>
                                                    
                                                    <td >
														@php
															$d=strtotime($response->updated_at);
															$date= date("h:i:s a",$d)
														@endphp
														{{$date}}
													</td>
                                                    ...
                                                    <td>
                                                        <a href="{{Url('admin/quiz_response_detail/'.$response->id)}}"><i class="far fa-list-alt" style="color: green; font-size: 18px;margin-right: 10px" aria-hidden="true"></i></a>
                                                    </td>
                                                </tr>
												@php
												 $count++;
		...
		<script>
			function delete_doc(id){
				...
				$.get('delete_quiz_docs/'+id).then((result)=>{
					Swal.fire({
						position: 'top-end',
						icon: result.type,
						title: result.msg,
